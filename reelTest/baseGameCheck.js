 ...
 * @returns {object} obj 亂數取出的 Array[5][3]
 */
getReelRandomFromBounsGame = (reelObj) => {
    var obj = {};
...
winNumberClassify = (winObj) => {
    ...
    for (var j = 0; j < keys.length; j++) {
        ...
        if (key == config.SPECIAL_NUMBER && config.FREE_GAME_CHECK_SWITCH) {
            totalObj['totalWinBonus'] = totalObj['totalWinBonus'] + thisReelLine + getFreeGameBouns(0);
        } else {
    ...
    for (var i = 1; i <= BounsGame1Obj['Times']; i++) {
        ...
        for (var j = 0; j < keys.length; j++) {
            ...
            if (key == config.SPECIAL_NUMBER) {
                freeGameBouns = getFreeGameBouns(freeGameBouns);
            }
